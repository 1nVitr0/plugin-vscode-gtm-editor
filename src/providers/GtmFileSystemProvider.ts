...
export class GtmFileSystemProvider implements FileSystemProvider {
  ...
  private getPath(uri: Uri): {
    accountId?: string;
    containerId?: string;
    folderOrItemType?: string;
    itemTypeOrId?: string;
    id?: string;
  } {
    ...
    if (accounts && accounts !== "accounts") throw new Error("Invalid path");
    if (containers && containers !== "containers") throw new Error("Invalid path");

    return { accountId, containerId, folderOrItemType, itemTypeOrId, id };
  }
